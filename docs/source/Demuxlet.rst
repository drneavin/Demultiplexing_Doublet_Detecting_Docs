...
.. tabs::
  ...
  .. tab:: With ``$INDS`` file
    ...
    .. code-block:: bash
      ...
      singularity exec Demuxafy.sif popscle dsc-pileup --sam $BAM --vcf $VCF --group-list $BARCODES ${CELL_TAG:+--tag-group $CELL_TAG} ${UMI_TAG:+--tag-UMI $UMI_TAG} --out $DEMUXLET_OUTDIR/pileup --sm-list $INDS
