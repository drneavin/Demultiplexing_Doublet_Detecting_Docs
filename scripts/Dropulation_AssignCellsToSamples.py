...
parser.add_argument("-I", "--INPUT_BAM", required = False, default = None, help = "Override NUM_CORE_BARCODES and process reads that have the cell barcodes in this file instead. The file has 1 column with no header. Required. Cannot be used in conjunction with argument(s) NUM_BARCODES.")
...
parser.add_argument("--ALLELE_FREQUENCY_ESTIMATE_FILE", required = False, default = 'null', help = "A file that contains an estimate of the allele frequency expected for each SNP across donors. The best estimate of this will come from the fraction of reference and alternate alleleUMIs that are observed at each snp site.  This report can be generated via GatherDigitalAlleleCounts.  This is a fractional estimate between 0 and 1.  File is tab seperated, with at least 3 columns:chromosome, position, maf_umi.  When supplied and CELL_CONTAMINATION_ESTIMATE_FILE is provided, this modifies the likelihood error rates to take into account how often the allele observed can be drawn from ambient RNA.  Default value: null.")
parser.add_argument("--ANSWER_KEY_FILE", required = False, default = 'null', help = "This file provides an answer key for the input BAM and VCF. This does not change any of the likelihood analysis results, but appends an additional column to the output that contains the known sample assignment for a cell.This is useful when assessing how well the method works, but is completely optional. The format of the file is 2 tab seperated columns with a header [cell sample]. The first column contains the cell barcodes from the BAM, the second the sample assignments from the VCF. Default value: null..")
parser.add_argument("--arguments_file", required = False, default = 'null', help = "read one or more arguments files and add them to the command line  This argument may be specified 0 or more times. Default value: null. .")
parser.add_argument("--BAM_OUTPUT", required = False, default = 'null', help = "Output a version of the BAM containing only the reads that have coverage in the input BAM and VCF.  Default value: null.")
parser.add_argument("--CELL_BARCODE_TAG", required = False, default = 'XC', type = int, help = "The cell barcode tag. If there are no reads with this tag, the program will assume that all reads belong to the same cell and process in single sample mode. Default value: XC.")
parser.add_argument("--CELL_CONTAMINATION_ESTIMATE_FILE", required = False, default = 'null', help = "A file that contains an estimate of how much ambient RNA is in each cell. This is a fractional estimate between 0 and 1. File is tab seperated, with 2 columns:cell_barcode and frac_contamination. When supplied along side the ALLELE_FREQUENCY_ESTIMATE_FILE, this modifies the likelihood error rates to take into account how oftenthe allele observed can be drawn from ambient RNA. We use cellbender remove background [https://github.com/broadinstitute/CellBender] to estimate the number of transcripts before and after ambient cleanup to define the fraction of transcripts that come from ambient RNA. Default value: null.")
...
parser.add_argument("--FIXED_ERROR_RATE", required = False, default = 'null', help = "Instead of using base qualities to determine error rate, use a fixed error rate instead. This is rounded to the nearest phread score internally.  Default value: null..")
...
parser.add_argument("--IGNORED_CHROMOSOMES", required = False, default = '[X, Y, MT]', help = "A list of chromosomes to omit from the analysis. The default is to omit the sex chromosomes. This argument may be specified 0 or more times. Default value: [X, Y, MT].")
parser.add_argument("--LOCUS_FUNCTION_LIST", required = False, default = '[CODING, UTR]', help = "A list of functional annotations that reads need to be completely contained by to be considered for analysis. This argument may be specified 0 or more times. Default value: [CODING, UTR]. Possible values: {INTERGENIC, INTRONIC, UTR, CODING, RIBOSOMAL}.")
parser.add_argument("--MAX_ERROR_RATE", required = False, default = 'null', help = "Caps the base error rate at a maximum probability so no SNP can be weighed more than this value. For example, if this value was 0.01, then a base quality 30 value (normally an erro rate of 0.001) would become 0.01. With the same threshold, a base with an error rate of 0.1 would be unaffected. Default value: null.")
...
parser.add_argument("--REFERENCE_SEQUENCE", required = False, default = 'null', help = "Reference sequence file.  Default value: null.")
parser.add_argument("--RETAIN_MONOMORPIC_SNPS", required = False, default = 'flase', help = "Should monomorphic SNPs across the population be retained?  Default value: false. Possible values: {true, false}.")
parser.add_argument("--SAMPLE_FILE", required = False, default = 'null', help = "A file with a list of samples in the VCF to match up to cells in the BAM. This subsets the VCF into a smaller data set containing only the samples listed. The file has 1 column with no header.  Default value: null.")
...
parser.add_argument("--TMP_DIR", required = False, default = 'null', help = "One or more directories with space available to be used by this program for temporary storage of working files. This argument may be specified 0 or more times. Default value: null.")
...
parser.add_argument("--VCF_OUTPUT", required = False, default = 'null', help = "Output a version of the VCF containing only the variants that have coverage in the input BAM. All samples are retained. This file can be used as the VCF input file on subsequent runs of the same data set to greatly speed up the run, if the same BAM is being analyzed with different conditions. If you plan on calling doublets with DetectDoublets, you'll need this file. Default value: null.")
parser.add_argument("--VERBOSE_BEST_DONOR_OUTPUT", required = False, default = 'null', help = "Verbose output of every cell/SNP result for the BEST donor. This supports zipped formats like gz and bz2. Default value: null.")
parser.add_argument("--VERBOSE_OUTPUT", required = False, default = 'null', help = "Verbose output of every cell/SNP result. This supports zipped formats like gz and bz2. Default value: null.")
parser.add_argument("--VERBOSITY", required = False, default = 'null', help = "display the version number for this tool  Default value: true. Possible values: {true,false}.")
...
if args.CELL_BC_FILE is None:
    if args.NUM_BARCODES is None:
        ...
        exit(0)
elif args.INPUT_BAM is None:
    print("Error: INPUT_BAM must be specified with CELL_BC_FILE.")
    exit(0)

subprocess.run("AssignCellsToSamples --CELL_BC_FILE " + args.CELL_BC_FILE + 
                " -I " + args.INPUT_BAM + 
                " -O " + args.OUTPUT + 
                " --NUM_BARCODES " + args.NUM_BARCODES +
                " --VCF " + args.VCF + 
                " --ADD_MISSING_VALUES " + args.ADD_MISSING_VALUES +
                " --ALLELE_FREQUENCY_ESTIMATE_FILE " + args.ALLELE_FREQUENCY_ESTIMATE_FILE +
                ...
                " --CELL_CONTAMINATION_ESTIMATE_FILE " + args.CELL_CONTAMINATION_ESTIMATE_FILE +
                " --COMPRESSION_LEVEL " + args.COMPRESSION_LEVEL +
                " --CREATE_INDEX " + args.CREATE_INDEX +
                " --CREATE_MD5_FILE " + args.CREATE_MD5_FILE +
                " --DNA_MODE " + args.DNA_MODE +
                " --EDIT_DISTANCE " + args.EDIT_DISTANCE +
                " --FIXED_ERROR_RATE " + args.FIXED_ERROR_RATE +
                " --FRACTION_SAMPLES_PASSING " + args.FRACTION_SAMPLES_PASSING +
                " --FUNCTION_TAG " + args.FUNCTION_TAG +
                " --GA4GH_CLIENT_SECRETS " + args.GA4GH_CLIENT_SECRETS +
                " --GENE_FUNCTION_TAG " + args.GENE_FUNCTION_TAG +
                " --GENE_NAME_TAG " + args.GENE_NAME_TAG +
                " --GENE_STRAND_TAG " + args.GENE_STRAND_TAG +
                " --GQ_THRESHOLD " + args.GQ_THRESHOLD +
                " --IGNORED_CHROMOSOMES " + args.IGNORED_CHROMOSOMES +
                " --LOCUS_FUNCTION_LIST " + args.LOCUS_FUNCTION_LIST +
                " --MAX_ERROR_RATE " + args.MAX_ERROR_RATE +
                " --MAX_RECORDS_IN_RAM " + args.MAX_RECORDS_IN_RAM +
                " --MOLECULAR_BARCODE_TAG " + args.MOLECULAR_BARCODE_TAG +
                " --QUIET " + args.QUIET +
                " --READ_MQ " + args.READ_MQ +
                " --REFERENCE_SEQUENCE " + args.REFERENCE_SEQUENCE +
                " --RETAIN_MONOMORPIC_SNPS " + args.RETAIN_MONOMORPIC_SNPS +
                " --SAMPLE_FILE " + args.SAMPLE_FILE +
                " --SNP_LOG_RATE " + args.SNP_LOG_RATE +
                " --STRAND_STRATEGY " + args.STRAND_STRATEGY +
                " --TMP_DIR " + args.TMP_DIR +
                " --TRANSCRIBED_SNP_FAIL_FAST_THRESHOLD " + args.TRANSCRIBED_SNP_FAIL_FAST_THRESHOLD +
                " --USE_JDK_DEFLATER " + args.USE_JDK_DEFLATER +
                " --USE_JDK_INFLATER " + args.USE_JDK_INFLATER +
                " --VALIDATION_STRINGENCY " + args.VALIDATION_STRINGENCY +
                " --VCF_OUTPUT " + args.VCF_OUTPUT +
                " --VERBOSE_BEST_DONOR_OUTPUT " + args.VERBOSE_BEST_DONOR_OUTPUT +
                " --VERBOSE_OUTPUT " + args.VERBOSE_OUTPUT +
                " --VERBOSITY " + args.VERBOSITY, shell = True)
