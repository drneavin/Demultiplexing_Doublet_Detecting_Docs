...
if (length(which(c(!is.null(args$demuxlet), !is.null(args$freemuxlet), !is.null(args$scSplit), !is.null(args$souporcell), !is.null(args$vireo),!is.null(args$DoubletDecon), !is.null(args$DoubletDetection), !is.null(args$DoubletFinder), !is.null(args$scDblFinder), !is.null(args$scds), !is.null(args$scrublet), !is.null(args$solo)))) < 1){
	...
	if (!is.null(args$demuxlet)){
		...
		if (file_test("-f", args$demuxlet)){
			...
			demuxlet <- list.files(pattern = ".best")
		...
		colnames(results_list[["demuxlet"]]) <- c("Barcode", "Demuxlet_Droplet_Type", "Demuxlet_Individual_Assignment")

		### update Demuxlet_Droplet_Type column SNG and DBL ###
		results_list[["demuxlet"]]$Demuxlet_Droplet_Type <- gsub("SNG", "singlet", results_list[["demuxlet"]]$Demuxlet_Droplet_Type) %>% gsub("DBL", "doublet", .) %>% gsub("AMB", "ambiguous", .)

		### Update Individual column to be individual or doublet or ambiguous/unassigned ###
		results_list[["demuxlet"]]$Demuxlet_Individual_Assignment <- ifelse(results_list[["demuxlet"]]$Demuxlet_Droplet_Type == "doublet", "doublet",
																		ifelse(results_list[["demuxlet"]]$Demuxlet_Droplet_Type == "unassigned", "unassigned", gsub(",.+,.\\..+","",results_list[["demuxlet"]]$Demuxlet_Individual_Assignment)))
	...
	if (!is.null(args$freemuxlet)){
		...
		if (file_test("-f", args$freemuxlet)){
			...
			freemuxlet <- list.files(pattern = "/samples.gz")
		...
		colnames(results_list[["freemuxlet"]]) <- c("Barcode", "Freemuxlet_Droplet_Type", "Freemuxlet_cluster")

		### update Demuxlet_Droplet_Type column SNG and DBL ###
		results_list[["freemuxlet"]]$Freemuxlet_Droplet_Type <- gsub("SNG", "singlet", results_list[["freemuxlet"]]$Freemuxlet_Droplet_Type) %>% gsub("DBL", "doublet", .) %>% gsub("AMB", "ambiguous", .)

		### Update Individual column to be individual or doublet or ambiguous/unassigned ###
		results_list[["freemuxlet"]]$Freemuxlet_cluster <- ifelse(results_list[["freemuxlet"]]$Freemuxlet_Droplet_Type == "doublet", "doublet",
																		ifelse(results_list[["freemuxlet"]]$Freemuxlet_Droplet_Type == "unassigned", "unassigned", gsub(",.+","",results_list[["freemuxlet"]]$Freemuxlet_cluster)))
	...
	if (!is.null(args$scSplit)){
		...
		### Update dataframe to just be categories of interest
		results_list[["scSplit"]][, c("scSplit_Droplet_Type", "scSplit_cluster") := tstrsplit(Cluster, "-", fixed=TRUE)]
		results_list[["scSplit"]]$Cluster <- NULL
		results_list[["scSplit"]]$scSplit_Droplet_Type <- gsub("SNG", "singlet", results_list[["scSplit"]]$scSplit_Droplet_Type) %>%
															gsub("DBL", "doublet", .) 
	...
	if (!is.null(args$souporcell)){
		...
		colnames(results_list[["souporcell"]]) <- c("Barcode", "Souporcell_Droplet_Type", "Souporcell_cluster")
		results_list[["souporcell"]]$Souporcell_cluster <- ifelse(results_list[["souporcell"]]$Souporcell_Droplet_Type == "doublet", "doublet", results_list[["souporcell"]]$Souporcell_cluster)
	...
	if (!is.null(args$vireo)){
		...
		if (file_test("-f", args$vireo)){
			...
			try:
				results_list[["vireo"]] <- fread(paste0(args$souporcell, "/donor_ids.tsv"), sep = "\t")
			except:
				message("Can't read in the vireo report either from the file you provided or to find the 'donor_ids.tsv' file in the directory you provided. Please double check your path and provide the full path to the vireo file.")
		}
		
		### Update dataframe to just be categories of interest
		results_list[["vireo"]] <- results_list[["vireo"]][,c("cell", "donor_id")]
		colnames(results_list[["vireo"]]) <- c("Barcode", "vireo_Individual_Assignment")
		results_list[["vireo"]]$vireo_Droplet_Type <- ifelse(results_list[["vireo"]]$vireo_Individual_Assignment == "doublet", "doublet", "singlet")
	...
	fwrite(combined_results, paste0(out, "/combined_results.tsv"), sep = "\t")
